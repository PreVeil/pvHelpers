...
class PreparedMessageFactory(object):
    ...
    def new(sender, email, recipient):
        try:
            if email.protocol_version == PROTOCOL_VERSION.V1:
                return True, PreparedMessageV1(sender, email, recipient)
            elif email.protocol_version == PROTOCOL_VERSION.V2:
                return True, PreparedMessageV2(sender, email, recipient)
            elif email.protocol_version == PROTOCOL_VERSION.V3:
                return True, PreparedMessageV3(sender, email, recipient)
            elif email.protocol_version == PROTOCOL_VERSION.V4:
        ...
        g_log.error(u"PreparedMessageFactory.new: Unsupported protocol_version")
        return False, None

...
class PreparedMessageV1(PreparedMessageHelpers, PreparedMessageBase):
    ...
    def __init__(self, sender, email, recipient):
        ...
        status, body_mime = EmailV1.replaceDummyReferences(body_mime, {HexEncode(Sha256Sum(att.content.content)): u",".join(att.content.block_ids) for att in self.email.attachments})
        if not status:
        ...
        if self.recipient.user_id == self.sender.user_id:
            ...
            if self.recipient.user_id in [bcc["user_id"] for bcc in self.email.bccs] or self.recipient.user_id not in [recip["user_id"] for recip in self.email.tos + self.email.ccs]:
                status, body_mime = EmailV1.setMIMEBcc(body_mime, [{"user_id": self.recipient.user_id, "display_name": self.recipient.display_name}])
            else:
    ...
    def canonicalEncryptedString(blocks):
        ...
        for block_id in sorted(blocks):
            ...
            if not isinstance(block, dict):
                g_log.error(u"PreparedMessageV1.canonicalEncryptedString: block must be of type dict")
                return False, None

            x = block.get("data")
            if not isinstance(x, unicode):
                g_log.error(u"PreparedMessageV1.canonicalEncryptedString: block data must be of type unicode")
                return False, None
            msg += x

...
class EmailBase(object):
    def __init__(self, server_attr, protocol_version, flags, tos, ccs, bccs, sender, reply_tos, \
                 subject, body,  attachments, references, in_reply_to, message_id):
        ...
        if not isinstance(sender, unicode):
            raise EmailException(u"EmailBase.__init__: sender must be of type unicode")
        self.sender = sender

        if not isinstance(tos, list):
            raise EmailException(u"EmailBase.__init__: tos must be of type list")
        for to in tos:
            if not isinstance(to, unicode):
                raise EmailException(u"EmailBase.__init__: tos element must be of type unicode")
        self.tos = tos

        if not isinstance(ccs, list):
            raise EmailException(u"EmailBase.__init__: ccs must be of type list")
        for cc in ccs:
            if not isinstance(cc, unicode):
                raise EmailException(u"EmailBase.__init__: ccs element must be of type unicode")
        self.ccs = ccs

        if not isinstance(bccs, list):
            raise EmailException(u"EmailBase.__init__: bccs must be of type list")
        for bcc in bccs:
            if not isinstance(bcc, unicode):
                raise EmailException(u"EmailBase.__init__: bccs element must be of type unicode")
        self.bccs = bccs

        if not isinstance(reply_tos, list):
            raise EmailException(u"EmailBase.__init__: reply_tos must be of type list")

        self.reply_tos = []
        for rpt in reply_tos:
            if not isinstance(rpt, unicode):
                continue

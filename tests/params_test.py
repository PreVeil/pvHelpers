...
def method_caller(*values):
    @params(int, str, unicode, {bool, int}, {dict}, [int], (int, str), float, {"xx": [int], "zz": {DummyClass}}, [{types.FunctionType}])
    def callMe(a, b, c, d, e, f, g, h, i, j):
        print "valid inputs"
...
def test_arg_values_checking():
    ...
    valid_params = (1, "str", u"unicode", 4, dict_of_dicts, list_of_ints, (3, "fixed size tuple"), 4.3, {"xx": list_of_ints, "zz": dict_of_dummies}, list_of_dict_of_callables)
    valid_params2 = (1, "str", u"unicode", True, dict_of_dicts, list_of_ints, (3, "fixed size tuple"), 4.3, {"xx": list_of_ints, "zz": dict_of_dummies}, list_of_dict_of_callables)
    ...
    pytest.raises(TypeError, method_caller, 1, "u", u"s", True, {"a": {"d": "d"}}, [1, 2], (1, dummy_instance), *valid_params[9:])
    # 3 is not of type float
    pytest.raises(TypeError, method_caller, 1, "u", u"s", 1231, {"a": {"d": "d"}}, [1, 2], (1, "s"), 3, *valid_params[10:])
    # missing key "xx"
    pytest.raises(KeyError, method_caller, 1, "u", u"s", False, {"a": {"d": "d"}}, [1, 2], (1, "s"), 3.0, {"a": 1}, *valid_params[11:])
    # ["str", 1] is not of type DummyClass
    pytest.raises(TypeError, method_caller, 1, "u", u"s", 123, {"a": {"d": "d"}}, [1, 2], (1, "s"), 3.0, {"xx": [], "zz": {"a": ["str", 1]}}, *valid_params[12:])
    # dummy_instance is not of type Function
    pytest.raises(TypeError, method_caller, 1, "u", u"s", 1232, {"a": {"d": "d"}}, [1, 2], (1, "s"), 3.0, {"xx": list_of_ints, "zz": dict_of_dummies}, [{"a": dummy_function, "v": dummy_instance}])

...
def test_create_mime_with_regular_attachment():
    ...
    raw_mime = create_mime(text, html, attachments)
    ...
    assert set(map(lambda att: att.metadata.content_disposition, attachments)) == \
        set(map(lambda att_part: att_part.content_disposition[0], attachment_parts))
    assert set(map(lambda att: att.metadata.filename, attachments)) == \
        set(map(lambda att_part: att_part.content_disposition[1]["filename"], attachment_parts))
    assert set(map(lambda att: att.content.content, attachments)) == \
        set(map(lambda att_part: att_part.body, attachment_parts))
    assert len(re.findall("=\n", raw_mime.to_string())) == 0
...
def test_create_mime_with_inline_and_regular_attachment():
    ...
    assert len(raw_mime.parts) == \
        len(filter(lambda att: att.metadata.content_disposition == AttachmentType.ATTACHMENT, attachments)) + 1
    assert "(multipart/alternative)" in map(lambda p: str(p), raw_mime.parts)
    # testing regular atts content
    assert set(map(
        lambda att: att.content.content,
        filter(lambda att: att.metadata.content_disposition == AttachmentType.ATTACHMENT, attachments)
    )) == set(map(
        lambda att_part: att_part.body,
        filter(lambda p: p.content_disposition[0] == AttachmentType.ATTACHMENT, raw_mime.parts)
    ))
    ...
    # one for html and rest inline atts
    assert len(related_part.parts) == \
        len(filter(lambda att: att.metadata.content_disposition == AttachmentType.INLINE, attachments)) + 1
    assert html == filter(lambda p: str(p) == "(text/html)", related_part.parts)[0].body
    ...
    assert set(map(
        ...
        filter(lambda att: att.metadata.content_disposition == AttachmentType.INLINE, attachments)
    )) == set(map(lambda att_part: att_part.content_disposition[1]["filename"], inline_attachment_parts))
    assert set(map(
        lambda att: att.content.content,
        filter(lambda att: att.metadata.content_disposition == AttachmentType.INLINE, attachments)
    )) == set(map(lambda att_part: att_part.body, inline_attachment_parts))
    assert len(re.findall("=\n", raw_mime.to_string())) == 0
...
def test_create_mime_headers_validity():
    ...
    assert tos == [
        {"user_id": to.address, "display_name": to.display_name}
        for to in addresslib.address.parse_list(raw_mime.headers.get("To"))
    ]
    assert ccs == [
        {"user_id": cc.address, "display_name": cc.display_name}
        for cc in addresslib.address.parse_list(raw_mime.headers.get("Cc"))
    ]
    assert bccs == [
        {"user_id": bcc.address, "display_name": bcc.display_name}
        for bcc in addresslib.address.parse_list(raw_mime.headers.get("Bcc"))
    ]
    assert reply_tos == [
        {"user_id": rpt.address, "display_name": rpt.display_name}
        for rpt in addresslib.address.parse_list(raw_mime.headers.get("Reply-To"))
    ]
...
def test_parse_mime_regular_attachment():
    ...
    assert map(lambda att: att.content.content, attachments) == \
        map(lambda att_part: att_part.body,
            filter(lambda p: p.content_disposition[0] == AttachmentType.ATTACHMENT, root_mime.parts))
...
def test_invalid_content_disposition():
    ...
    assert attachments[0].content.content == \
        filter(lambda p: p.content_disposition[0] == u"invalidCD", root_mime.parts)[0].body
...
def test_unknown_content_type_with_no_disposition():
    ...
    # all attachment still has the right content
    assert map(lambda att: att.content.content, attachments) == \
        map(lambda p: p.body, filter(lambda p: p.content_type.value != u"text/plain", root_mime.parts))
